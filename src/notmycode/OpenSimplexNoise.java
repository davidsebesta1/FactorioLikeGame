...
public class OpenSimplexNoise {
	...
	public static double smoothNoise(double x, double y, OpenSimplexNoise noise, double noiseSize) {
	    // Determine the four corner points of the square
	    int x0 = (int) Math.floor(x / noiseSize);
	    int y0 = (int) Math.floor(y / noiseSize);
	    int x1 = x0 + 1;
	    int y1 = y0 + 1;

	    // Determine the fractional part of the coordinates
	    double xs = x / noiseSize - x0;
	    double ys = y / noiseSize - y0;

	    // Interpolate horizontally between the two points on the bottom
	    double v1 = interpolate(
	        noise.eval(x0, y0, 0),
	        noise.eval(x1, y0, 0),
	        xs
	    );

	    // Interpolate horizontally between the two points on the top
	    double v2 = interpolate(
	        noise.eval(x0, y1, 0),
	        noise.eval(x1, y1, 0),
	        xs
	    );
