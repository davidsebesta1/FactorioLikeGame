		...
		for (int i = 0; i < manager.getChunkMap().length; i++) {
			for (int j = 0; j < manager.getChunkMap()[i].length; j++) {
				if (manager.getActiveChunks().contains(manager.getChunkMap()[i][j])) {
					g2d.drawImage(manager.getChunkMap()[i][j].getBuffer(), (i * (int) Game.getInstance().getCurrentWorld().getChunkManager().getChunkSize()
							- (int) Game.getInstance().getCurrentWorld().getPlayer().getCamera().getLocation().getX()),
							(j * (int) Game.getInstance().getCurrentWorld().getChunkManager().getChunkSize() - (int) Game.getInstance().getCurrentWorld().getPlayer().getCamera().getLocation()
									.getY()),
							null);
				}
			} 
